...
// Got these addresses from:
// https://github.com/Mirror-Protocol/whitelist/blob/master/tequila.json
const MIR_LP_staking = 'terra1a06dgl27rhujjphsn4drl242ufws267qxypptx';
const MIR_LP = 'terra1zrryfhlrpg49quz37u90ck6f396l4xdjs5s08j';
class Repay {
    constructor() {
    ...
    async repay(amount) {
        ...
        let repay = new MsgExecuteContract(this.wallet.key.accAddress, market, {
            repay_stable: {}
        }, coins);
    ...
    async withdraw_aUST(aust_amount) {
        ...
        let withdraw = new MsgExecuteContract(this.wallet.key.accAddress, aUST, {
            send: {
                contract: market,
    ...
    async unstake_ANC_LP(LP_token_amount) {
        ...
        let unstake = new MsgExecuteContract(this.wallet.key.accAddress, ANC_token, {
            unbond: {
    ...
    async withdraw_ANC_LP(LP_token_amount) {
        ...
        let withdraw = new MsgExecuteContract(this.wallet.key.accAddress, ANC_LP, {
            send: {
                contract: ANC_pool,
                amount: LP_token_amount.toString(),
    ...
    async unstake_mAsset_LP(LP_token_amount, token, symbol) {
        ...
        let unstake = new MsgExecuteContract(this.wallet.key.accAddress, MIR_LP_staking, {
            unbond: {
                asset_token: token,
                amount: LP_token_amount.toString()
            }
    ...
    async withdraw_mAsset_LP(LP_token_amount, token_data, symbol) {
        ...
        let withdraw = new MsgExecuteContract(this.wallet.key.accAddress, token_data.lpToken, {
            send: {
    ...
    async withdraw_bLUNA(amount) {
        ...
        let unlock = new MsgExecuteContract(this.wallet.key.accAddress, overseer, {
            unlock_collateral: {
                collaterals: [[bLUNA_token, amount.toString()]]
            }
        }, new Coins);
        let withdraw = new MsgExecuteContract(this.wallet.key.accAddress, custody, {
            withdraw_collateral: {
    ...
    async instant_burn(burnamount, max_premium_rate) {
        ...
        let swap = new MsgExecuteContract(this.wallet.key.accAddress, bLUNA_token, {
            send: {
    ...
    async luna2Ust(swapamount) {
        ...
        let swap = new MsgExecuteContract(this.wallet.key.accAddress, 'terra1vs9jr7pxuqwct3j29lez3pfetuu8xmq7tk3lzk', {
            assert_limit_order: {
                offer_coin: {
                    denom: "uluna",
        ...
        let swapMsg = new MsgSwap(this.wallet.key.accAddress, new Coin('uluna', swapamount), "uusd");
        await this.execute([swap, swapMsg]);
    }
    async provide_bLUNA(provide_amount) {
        let deposit = new MsgExecuteContract(this.wallet.key.accAddress, bLUNA_token, {
            send: {
                contract: custody,
        ...
        let lock = new MsgExecuteContract(this.wallet.key.accAddress, overseer, {
            lock_collateral: {
                collaterals: [
    ...
    async borrow_ust(ust_amount) {
        ...
        let borrow = new MsgExecuteContract(this.wallet.key.accAddress, market, {
            borrow_stable: {
...
async function aUST_process(insufficientUST) {
    ...
    if (insufficientUST > withdrawable_value) {
        ...
        aUST_amount = insufficientUST / exchange_rate;
    }
    if (aUST_amount != 0) {
...
async function ANC_LP_process(insufficientUST) {
    ...
    let LP_needed = insufficientUST / USTperLP;
...
async function instant_burn_process(nowPercent) {
    ...
    if (before_luna_balance == after_luna_balance) { //canceled swap b/c of luna/bluna premium
        await repayHandler.provide_bLUNA(withdraw_amount); //reprovide collateral
    }
    else {
